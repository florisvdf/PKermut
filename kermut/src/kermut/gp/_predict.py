...
def predict(
    ...
    with torch.no_grad():
        ...
        if preferential:
            # Using pw likelihood will attempt to obtain distributions over pairs rather than utility.
            # Then D matrix is required. Just get posterior.
            if training_data:
                gp.train()
                y_preds_dist = gp(gp.datapoints)
                gp.eval()  # Set back to eval after making predictions
            else:
                y_preds_dist = gp.posterior(*x_test)
